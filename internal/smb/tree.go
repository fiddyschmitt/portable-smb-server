...
// handleTreeConnect handles an SMB2 TREE_CONNECT request. We accept a
// connection to any of our configured shares and reject everything else with
...
func (c *conn) handleTreeConnect(h header, body []byte) (status uint32, treeID uint32, respBody []byte) {
	...
	shareName := shareFromPath(path)

	// Accept our disk shares and the special IPC$ share. cifs (Linux) connects
	// to IPC$ for DFS referral resolution and refuses to mount without it.
	var shareType byte
	sh := c.server.findShare(shareName)
	switch {
	case sh != nil:
		shareType = shareTypeDisk
	case strings.EqualFold(shareName, "IPC$"):
	...
	if shareType == shareTypePipe {
		...
		// resolved as filesystem paths (a real file named e.g. srvsvc must not be
		// opened).
