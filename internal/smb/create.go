...
type openFile struct {
	fileID        [16]byte
	share         *share   // the share the open belongs to
	path          string   // share-relative path (forward-slash, no leading slash)
	handle        fsx.File // open IO handle for files (nil for directories)
	isDir         bool
	deleteOnClose bool
...
// stat returns the current file information for the open. It is looked up
// live (not snapshotted at open time) so that size and mtime reflect writes
// made through the handle.
func (of *openFile) stat() (os.FileInfo, error) {
	if of.handle != nil {
		return of.handle.Stat()
	}
	return of.share.fs.Stat(of.path)
}

// attrs returns the SMB file attributes, size and modification time of the
// open, falling back to bare attributes if the stat fails (e.g. the file was
// deleted underneath us).
func (of *openFile) attrs() (attrs uint32, size int64, mtime time.Time) {
	fi, err := of.stat()
	if err != nil {
		if of.isDir {
			return fileAttrDirectory, 0, time.Time{}
		}
		return fileAttrNormal, 0, time.Time{}
	}
	return fileInfoAttrs(fi), fi.Size(), fi.ModTime()
}

// name returns the base name of the open's path ("" for the share root).
func (of *openFile) name() string {
	if of.path == "" {
		return ""
	}
	return path.Base(of.path)
}
...
// directory on the share connected as treeID and adds it to the connection's
// handle table.
func (c *conn) handleCreate(h header, body []byte, treeID uint32) (uint32, []byte) {
	if len(body) < 56 {
	...
	sh := c.diskTrees[treeID]
	if sh == nil {
		return statusNetworkNameDeleted, errorResponseBody()
	}
	...
	case dispOpen, dispOverwrite:
		if !exists {
			return mapOSError(statErr), errorResponseBody()
		}
	case dispCreate:
		if exists {
	...
	case exists && fi.IsDir():
		...
		of = &openFile{share: sh, path: path, isDir: true}
		action = actionOpened
	case !exists && createOptions&optDirectoryFile != 0:
		if err := sh.fs.Mkdir(path, 0777); err != nil {
	...
	case exists && !fi.IsDir() && createOptions&optDirectoryFile != 0:
	...
	default:
		...
		of = &openFile{share: sh, path: path, handle: handle}
		switch {
		case !exists:
			action = actionCreated
...
// os.OpenFile flags used by the filesystem backend.
...
// decoded) into a backend path (forward-slash, no leading or trailing slash).
// It cleans "." and ".." components and anchors the result at the share root so
// a client cannot escape the share with "..".
...
// joinPath joins a share-relative directory path and a child name.
...
// listDir returns the directory entries at the given path on a share.
func (c *conn) listDir(sh *share, path string) ([]os.FileInfo, error) {
	return sh.fs.ReadDir(path)
}

// mapOSError maps an os error to the closest NTSTATUS code.
func mapOSError(err error) uint32 {
	...
	default:
		...
		// Windows sharing violation on a file another process holds open, or
		// removing a non-empty directory). Mapping these to a precise status
		// rather than the STATUS_UNSUCCESSFUL catch-all lets SMB clients skip the
		// item instead of aborting the operation.
	...
	for _, of := range handles {
		if of.handle != nil {
			if err := of.handle.Close(); err != nil {
				errorf("SMB: error closing handle on teardown: %v", err)
			}
		}
	}
}
