...
// NewServer creates a new SMB server listening on opt.ListenAddr. The listener
// is opened immediately so that Addr reports the bound address (which matters
...
func NewServer(opt Options) (*Server, error) {
	...
	if len(opt.Shares) == 0 {
		return nil, errors.New("smb: at least one share is required")
	}
	s := &Server{
		opt:     opt,
		conns:   map[*conn]struct{}{},
	...
	for _, def := range opt.Shares {
		if def.Name == "" || def.FS == nil {
			return nil, errors.New("smb: every share needs a name and a filesystem")
		}
		if strings.EqualFold(def.Name, "IPC$") {
			return nil, errors.New("smb: the share name IPC$ is reserved")
		}
...
// findShare returns the share with the given name (case-insensitive), or nil.
func (s *Server) findShare(name string) *share {
...
func (s *Server) Serve() error {
	...
	for {
		...
		if err != nil {
			...
			errorf("SMB: accept error, retrying in %v: %v", acceptDelay, err)
			time.Sleep(acceptDelay)
			continue
		}
		acceptDelay = 0
		debugf("SMB: accepted connection from %s", nc.RemoteAddr())
		c := newConn(s, nc)
		s.mu.Lock()
		select {
		case <-s.closing:
...
type conn struct {
	...
	diskTrees          map[uint32]*share   // TreeIds of connected disk shares
...
func (c *conn) serve() {
	defer func() {
		// A panic in one request handler must not take down the whole server
		// process; log it and drop just this connection.
		if r := recover(); r != nil {
			errorf("SMB: recovered from panic serving %s: %v", c.nc.RemoteAddr(), r)
		}
		c.closeAllHandles()
		_ = c.nc.Close()
	}()
	for {
		...
		if err != nil {
			debugf("SMB: connection from %s closed: %v", c.nc.RemoteAddr(), err)
			return
		}
		...
		if err != nil {
			debugf("SMB: dispatch error: %v", err)
			return
		}
		if out != nil {
...
func (c *conn) dispatch(msg []byte) ([]byte, error) {
	...
	if _, ok := parseHeader(msg); !ok {
		...
		debugf("SMB: unparseable message len=%d first bytes: % x", len(msg), msg[:n])
...
func (c *conn) handleCommand(h header, seg []byte, ctx *chainCtx) []byte {
	...
	debugf("SMB >> cmd=%d msgid=%d flags=0x%x len=%d", h.command, h.messageID, h.flags, len(seg))
	...
	if c.server.opt.User != "" && h.command != cmdNegotiate && h.command != cmdSessionSetup {
		if _, ok := c.authedSessions[sessionID]; !ok {
			debugf("SMB: rejected cmd=%d on unauthenticated session %d", h.command, sessionID)
	...
	if c.signKey != nil && h.flags&flagsSigned != 0 && !verifyMessage(c.signKey, c.dialect, seg) {
		debugf("SMB: rejected cmd=%d with a bad signature on session %d", h.command, sessionID)
	...
	case cmdTreeDisconnect:
		delete(c.pipeTrees, treeID)
		delete(c.diskTrees, treeID)
	...
	case cmdCreate:
		if _, isPipe := c.pipeTrees[treeID]; isPipe {
			// We don't serve named pipes; opening one must fail rather than hit the
			// filesystem.
			status, respBody = statusObjectNameNotFound, errorResponseBody()
		} else {
			status, respBody = c.handleCreate(h, body, treeID)
	...
	debugf("SMB << cmd=%d status=0x%08x bodylen=%d", h.command, status, len(respBody))
